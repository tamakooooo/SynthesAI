...
class BilibiliDownloader(Downloader):
    ...
    def download_video(
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            candidates = []
            if video_id:
                candidates.extend(Path(output_dir).glob(f"{video_id}_video.*"))
            prepared_name = ydl.prepare_filename(info)
            if prepared_name:
                candidates.append(Path(prepared_name))
            ...
            if not file_path:
                raise RuntimeError("视频下载完成但未找到输出文件")
        ...
        try:
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                subtitle_file = os.path.join(
                    output_dir, f"{video_id}.{detected_lang}.{ext}"
                )
    ...
    def _parse_srt_content(
        self, srt_content: str, language: str
    ) -> TranscriptResult | None:
        """解析 SRT 格式字幕"""
        try:
            segments = []
